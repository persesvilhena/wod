...
<?php
session_start();
$mensagem_erro = "<b>Digite seu login e senha de admin</b>";
...
if(isset($_POST["logar"])) { // Verifico se o botão de login foi acionado
	...
	if(!empty($_POST["login"]) && !empty($_POST["senha"])) { // Verifico se os campos foram preenchidos
		...
		$valida_user = mysql_query("SELECT * FROM admin WHERE nome='$login' AND senha='$senha'") or die(mysql_error()); // Faço a consulta ao SQL para buscar o usuário com os dados informados pelo form
